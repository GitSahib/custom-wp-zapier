...
class UserActivity_User{
	...
	public function set_profile_data(){
		$this->avatar = get_avatar( $this->ID, 200, '', '', array('force_display'=>true));

		$this->coordinates = UserActivity_Helper::coordinates_string_to_array($this->coordinates);
	...
	protected function set_default_data($data){
		...
		foreach($data as $key => $val){
			if(!empty($val) && in_array($key, $date_fields) && $key != 'last_seen' && !$this->should_ignore_date_format){
				$this->$key = UserActivity_Helper::format_date($val);
			}else{
				$this->$key = $val;
			}
		}
		
		if(UserActivity_Options::is_field_visible('role')){
			$this->set_role();
		}
		$this->format_last_seen();
		if(UserActivity_Options::is_field_visible('user_groups')){
			$this->set_user_groups();
		}
	}
	...
	protected function format_last_seen(){
		if(!$this->should_ignore_date_format && isset($this->last_seen)){
			...
			$this->last_seen = empty($last_seen) ? '' : UserActivity_Helper::format_date_human($last_seen);
	...
	protected function set_user_groups(){
		if(!isset($this->user_groups)){
			$this->user_groups = UserActivity_Field_Defaults::get_user_groups($this->ID);
		}
	}
	
	protected function set_avatar(){
		$this->avatar = get_avatar( $this->ID, 96, '', '', array('force_display'=>true) );
	}
	
	protected function set_comments(){
		if(UserActivity_Options::is_field_visible('comments')){
	...
	protected function get_date_fields(){
		if(self::$date_fields === null){
			global $usin;
			self::$date_fields = UserActivity_Options::get_field_ids_by_operator_type('date');
		}
